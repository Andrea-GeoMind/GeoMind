...
Tous sprints terminés, Sprint 7 (V2 produit) inclus : crédits universels, plans V2 (Gratuit/Solo/Pro/Business + annuel), règles GEO V2 (57 checks + page par page + opportunités), GEO le coach IA (flottant + mémoire + Sonnet). geomind.fr est en production sur Vercel (cdg1). Reste : Stripe Live (bloqué SIRET — créer 6 prix plans + 3 packs et renseigner les env vars), Perplexity (déféré).
...
- [x] **TKT-CREDITS** — Système de crédits universel — spec `cahier-des-charges.md` §17 — `credit_balances` + `credit_transactions`, packs Stripe 5€/15€/49€, bonus bienvenue 1000 cr, reset à date anniversaire de facturation, remboursement auto si analyse échoue, badge sidebar + alerte 20% + équivalences humaines (2026-06-12)
- [x] **TKT-PLANS-V2** — Nouveau modèle Gratuit / Solo 19€ / Pro 59€ / Business 149€ (+ annuel -20% : 15/47/119) — spec §17 — feature gates (page analysis, mémoire GEO, full reco, publishers, PDF + white-label Business, support prioritaire), downgrade = sites gelés lecture seule, migration sans grandfathering (2026-06-12)
- [x] **TKT-COACH-V2** — GEO, le coach IA : refonte complète en 4 lots — spec `cahier-des-charges.md` §16 — bouton flottant global, "Demander à GEO" depuis les issues, mémoire par site avec compression, Sonnet + web search + fallback Haiku, chips contextuelles, comparaison N vs N-1, auto-ouverture unique, décompte crédits au réel (2026-06-12)
- [x] **TKT-RULES-V2** — Règles GEO v2 en 4 lots (V2a Infra / V2b Technique / V2c Contenu / V2d UI) — spec `cahier-des-charges.md` §18 — 31 règles techniques + 26 contenu actives (réintégration des 16 désactivées + 30 nouvelles), analyse page par page, scoring V2, opportunités garanties ≥ 3, quick wins (2026-06-12)
...
| TKT-COACH-V2 | ✅ | 2026-06-12 | GEO le coach IA (§16) : table coach_memory + sites.coach_intro_seen (migration 0008 appliquée), system prompt v2 (identité GEO, voix calibrée few-shot, anti-injection <donnees_site>, règles hors-sujet/invention), contexte enrichi (keywords/concurrents/top 5 issues par sévérité/mémoire/diff N vs N-1 avec issues résolues + flag méthodologie), route Sonnet 4.6 + web search OpenRouter + fallback Haiku « (mode rapide) », crédits au réel (30 upfront, ajustement usage tokens, plancher 5/plafond 60, remboursement si échec), rate limit 30 msg/h, input 2000 car., compression mémoire Inngest (Haiku, 4 sections, 800 car., tous les 10 messages), Server Actions intro-seen + effacer mémoire RGPD, UI : GeoAvatar orbe SVG, CoachProvider global, bouton + panel flottants (400px/plein écran mobile, persiste à la navigation), markdown maison avec bouton Copier, chips getSuggestions par page, « Demander à GEO » sur les IssueCards, auto-ouverture unique post-1re analyse, onglet GEO dans SiteTabs, accueil contextualisé, teaser mémoire plan Free, events PostHog — 511 tests verts (+8) — backend main agent, UI subagent |
| TKT-RULES-V2 | ✅ | 2026-06-12 | Règles GEO V2 (§18) : infra (geo-rules.ts sévérités major12/moderate6/minor3/opportunity0, computeIssuesScore avec proportionnalité page + plafond 30/catégorie, selectPagesForAnalysis déterministe, opportunities.ts garantie ≥3, migration 0007 appliquée : severity/effort/impact/page_url + analyses.rules_version), catalogue technique : 15 site + 16 page = 31 règles (6 réactivées dont response-time durci 2s, 14 nouvelles : canonical/OG/twitter/html-lang/viewport/noindex/images-alt/URLs/breadcrumb/author/howto/broken-links/pagination ; semantic-html5+redirect-chain+images-heavy droppées, données Firecrawl indisponibles), catalogue contenu : 16 site + 10 page = 26 règles (4 réactivées, 16 nouvelles : first-paragraph-answer/stats/citations-externes/fraîcheur/entités/duplicate-jaccard/comparatif/about/contact/author-bio/headings/phrases/keywords/conclusion/TOC, thin-content durci médiane+150), UI : quick wins (effort1+impact≥2), accordéons par page, section « Pour aller plus loin », badge « Méthodologie enrichie » — 503 tests verts (+136) — exécuté avec 2 subagents parallèles (technique/contenu) |
| TKT-PLANS-V2 | ✅ | 2026-06-12 | Nouveau modèle de plans (§17.2) : enum 'solo' (migration 0006 appliquée), PLAN_LIMITS V2 (free 1/500, solo 2/5k, pro 5/20k, business 15/80k), PLAN_FEATURES (pageAnalysisLimit, coachMemory, fullRecommendations, recommendationSheets, publishersFull, pdfExport, historyDays), PLAN_PRICES mensuel+annuel -20%, Stripe 6 prix (4 env vars optionnelles), createCheckoutSession(plan, period), top-up crédits à l'upgrade (webhook), gates appliqués : reco complète Pro+ (était Business), fiches Solo+, publishers Solo+, helpers quotas (getPageAnalysisLimit/canUseCoachMemory/canUseFullRecommendations pour RULES-V2 et COACH-V2), sites gelés au downgrade (computeFrozenSiteIds pure + badge dashboard + blocage analyse), pricing refondu (4 plans, toggle annuel, équivalences crédits, FAQ crédits 7 questions), billing multi-upgrade — pdfExport/historyDays définis mais non branchés (features à venir) — 367 tests verts (+13) |
| TKT-CREDITS | ✅ | 2026-06-12 | Système de crédits universel (§17) : tables credit_balances/credit_transactions + RLS (migration 0005 appliquée), lib/credits.ts (consommation atomique UPDATE conditionnel, lazy init bonus 1000 cr, reset lazy free/webhook payants), lib/credits-shared.ts (helpers purs client-safe), quotas.ts délègue aux crédits, analyse 400 cr décomptés au lancement + remboursement auto (action + Inngest), coach 10 cr/message + accès Free débloqué, packs Stripe (checkout mode=payment + webhook idempotent + 3 env vars optionnelles), reset mensuel sur invoice.payment_succeeded, badge crédits sidebar, bannière+email alerte 20%, billing (jauges + packs), usage (jauges + historique transactions), confirmation avant dépense sur RunAnalysisButton — 353 tests verts (+11) |
| REFONTE-2026 | ✅ | 2026-06-13 | Refonte complète post-audit (PLAN.md, 36/38 items, PRs #37→#49). Vague 0 assainir : cohérence pricing V2, RLS 12 tables, anti prompt-injection, idempotence Stripe, SEO geomind.fr (robots/sitemap/JSON-LD/OG), erreurs onboarding visibles, honnêteté méthodo. Vague 1 fiabilité+récurrence : mesure autorité 10 prompts + double mode + citation_checks, surveillance auto crons + alertes email, rapport mensuel, onglets Suivi + Plan d'action (vérif auto), Zod réponses moteurs, vulgarisation UI, mobile. Vague 2 acquisition : audit express public sans inscription, landing personas, blog 3 guides + À propos + llms.txt, générateur llms.txt, export PDF, essai Pro 7j. Vague 3-4 différenciation : Studio de correctifs, onglet Concurrents (share of voice), Pixel GeoMind (preuve ROI), Réputation (hallucinations), Local, Agent-Ready, OAuth Google, audit_logs, présentation tarifaire rebranchée. 13 onglets/site, 566 tests verts, migrations 0009-0018. Restant : AI Overviews (clé SERP), corrélations propriétaires (volume), plan Agence/Baromètre/Mistral (post-PMF), hausse tarifaire (post-traction) |
| QA-FIXES-0613 | ✅ | 2026-06-13 | Simulation parcours client complet (geomind.fr) + 6 correctifs déployés : (1) coach IA 500 — tiret cadratin « — » dans le header HTTP X-Title → fetch jetait « ByteString », header repassé en ASCII (PR #51) ; (2) coach répondait hors-contexte — prompt système envoyé dans un champ `system` ignoré par OpenRouter → passé en message role:'system' comme structured.ts (PR #52) ; (3) onglet Suivi 500 — objet Date brut dans db.execute(sql) → `since.toISOString()` dans getCitationTrend (PR #51) ; (4) onglets settings dupliqués (layout + chaque page) → retirés des pages (PR #51) ; (5) analyse offerte allégée gatée sur plan free (4 IA conservées, 3 questions, sans 2ᵉ salve spontanée → ~0,50€ au lieu de ~2€ ; payants+admin inchangés) (PR #54) ; (6) packs de crédits réservés aux abonnés (cohérence « crédits inutilisables sans abonnement ») — section masquée pour le plan free + wording Tarifs/FAQ corrigé (PR #55). Mode test admin introduit puis retiré sur demande (PR #53). Causes diagnostiquées en reproduisant les requêtes contre la prod. 566 tests verts, /full-audit vert à chaque push — PRs #51→#55 |
| UX-NAV-0613 | ✅ | 2026-06-13 | Refonte navigation fiche site + retrait Agent-Ready (push direct main). (1) 14 onglets horizontaux → sidebar verticale groupée en 5 sections (Vue d'ensemble / Agir / Diagnostic / Marché / Résultats), fallback barre horizontale scrollable sur mobile (`site-nav.tsx` remplace `site-tabs.tsx`). (2) Feature Agent-Ready (ex-item 33) supprimée entièrement : route `agent-ready/`, `lib/analysis/agent-ready.ts`, test, onglet, carte landing, ligne+plan du tableau pricing, mentions docs (PLAN/AUDIT-STRATEGIE/PROPOSITION-TARIFAIRE) — jugée trop en avance pour la cible TPE. (3) Résumés remontés sur la Vue d'ensemble : carte « Visibilité IA · 30 j » (taux citation, lien Suivi) + carte « Trafic venu des IA · 30 j » (visiteurs+actions, lien Pixel, si pixel actif). 13 onglets/site, 561 tests verts, /full-audit vert |
| UX-FIXES-0613 | ✅ | 2026-06-13 | Fusion Studio → correctifs contextualisés + engagement coach (push direct main, suite de UX-NAV-0613). (1) Onglet Studio supprimé (page+panel) ; `lib/analysis/studio.ts` conservé. Nouveau `lib/analysis/action-fixes.ts` (mapping ruleKey → correctif) + `action-fixes.server.ts` (helper serveur mutualisé metadata+pages+CMS). (2) Composant partagé `components/features/fixes/fix-details.tsx` : bloc « Voir le correctif prêt à coller » (code + copier/télécharger + instruction CMS), affiché dans le Plan d'action ET les fiches reco Technique/Contenu — ouvert à tous les plans (la reco IA détaillée reste gatée Pro/Business). (3) Couverture correctifs élargie : fichiers générés (llms.txt, robots.txt, schema Org/FAQ, meta accueil) + 7 templates statiques (H1, lang, viewport, canonical, Open Graph, Twitter Card, alt images). (4) Bouton « Demander à GEO de m'aider » ajouté dans les fiches → ouvre GEO et auto-envoie un 1er message contextuel (mécanisme focusedIssue, jusque-là atteignable seulement depuis la carte). (5) « Comment corriger » générique masqué quand un correctif prêt à coller le remplace. Textes recâblés (Concurrents, checklist Local, landing+pricing : « correctifs prêts à coller »). 12 onglets/site, 566 tests verts, /full-audit vert |
