...
export function ContentIssuesList({ issues, isPro, isBusiness, fixesByRule }: ContentIssuesListProps) {
  ...
  const byPage = new Map<string, ContentIssueRow[]>()
        ...
        {realIssues.length === 0 && (
          <div className="flex flex-col items-center gap-3 rounded-2xl border border-[--score-good-200] bg-[--score-good-50] px-6 py-12 text-center">
            ...
            <p className="text-sm text-[--score-good-700]/80">
              Votre contenu respecte toutes les règles GEO analysées. Jetez un œil aux pistes
          ...
          <section>
            ...
            <div className="space-y-2">
              {pageGroups.map(([url, items]) => (
                <details key={url} className="group rounded-xl border border-border bg-card">
